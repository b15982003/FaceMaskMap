...
//
//  CoreDataBackup.swift
//  CGM
//
//  Created by Jerome on 2018/10/17.
//  Copyright © 2018 BionimeIOT. All rights reserved.
//

// From: https://gist.github.com/ole/e113a716158e26c1089a1d74b468deed
...
extension NSPersistentStoreCoordinator {
    func backupPersistentStore(atIndex index: Int, filename: String) throws -> TemporaryFile {
        ...
        // Filename format: sqlite file name (time is in UTC)
        func makeFilename(_ name: String) -> String {
            return name + ".sqlite"
        }

        let backupFilename = makeFilename(filename)
    ...
    func urlForUniqueTemporaryDirectory(preferredName: String? = nil) throws -> (url: URL, deleteDirectory: () throws -> Void) {
        ...
        repeat {
            do {
                ...
                let myTemporaryDirectory = URL(fileURLWithPath: NSTemporaryDirectory())
                let subdirectory = myTemporaryDirectory.appendingPathComponent(subdirName, isDirectory: true)
